...
GO
 ...
 AS
 SELECT serial_no, product_name, category, product_description, price,final_price, color, available, rate, vendor_username, customer_username 
 FROM Product
 ORDER BY price ASC
 GO
 --drop proc ShowProductsbyPrice
...
CREATE PROC addToCart
 ...
 AS
 if(exists(SELECT * FROM Product p WHERE p.available='1'))
 BEGIN
...
@customername VARCHAR(20),@orderid INT OUTPUT,@total_amount Decimal(10,2) OUTPUT
AS
...
--DECLARE @priceOrder DECIMAL(10,2)
--DECLARE @order_id INT
--EXEC makeOrder 'ahmed.ashraf',@order_id output,@priceOrder output
--print @order_id
--print @priceorder
...
------------------------------
CREATE PROC check_order 
@orderID int ,@success bit OUTPUT 
AS
...
set @success='0';
 ...
 GO
----------------------------
CREATE PROC order_status
@orderID int,@status INT OUTPUT
AS
if(exists(select * FROM Orders WHERE order_no = @orderid AND ( order_status = 'delivered' )))
SET @status='2';
else if(exists(select * FROM Orders WHERE order_no = @orderid AND ( order_status = 'out for delivery' )))
SET @status='0';
else
SET @status='1';
...
FROM Orders o
WHERE o.order_no=@orderid
if(exists(select * from Customer_CreditCard c where c.customer_name=@customername and c.cc_number=@creditcard))
BEGIN
UPDATE Orders 
...
GO
------------------------------
CREATE PROC check_card
@creditcard VARCHAR(20) , @customername VARCHAR(20) , @success BIT OUTPUT
AS
if(exists(select * from Customer_CreditCard c where c.customer_name=@customername and c.cc_number=@creditcard))
BEGIN
SET @success='1';
...
--------
GO
CREATE proc cart_is_empty
@customername varchar(20),@is_empty bit OUTPUT
AS
if(not exists(SELECT * FROM CustomerAddstoCartProduct c WHERE c.username=@customername))
set @is_empty ='1';
else
set @is_empty='0';
go
...
CREATE PROC checkCreditCard
 @creditcardnumber VARCHAR(20), @cvv VARCHAR(4), @customername VARCHAR(20),@check BIT OUTPUT
 AS
 IF(EXISTS(SELECT * FROM Credit_card c WHERE @creditcardnumber=c.number))
 SET @check='0'
 ELSE 
 SET
 @check='1'
 -------------------------
 GO
 CREATE PROC checkRemovefromWishlist
 @customername VARCHAR(20), @wishlistname VARCHAR(20), @serial int, @check INT OUTPUT
AS
if (EXISTS (SELECT * FROM Wishlist_Product 
WHERE customer_username=@customername AND wish_name=@wishlistname AND serial_no=@serial ))
SET @check='0'
ELSE
BEGIN
...
SET @check='1'
ELSE IF NOT (EXISTS (SELECT * FROM Wishlist W 
...
----------------------------
GO
CREATE PROC checkAddToWishlist
 @customername VARCHAR(20), @wishlistname VARCHAR(20), @serial int, @check INT OUTPUT
AS
if (EXISTS (SELECT * FROM Wishlist w
WHERE w.username=@customername AND w.name=@wishlistname)AND (EXISTS(SELECT * FROM Product
WHERE serial_no=@serial))AND NOT(EXISTS(SELECT * FROM Wishlist_Product W
WHERE W.customer_username=@customername AND W.wish_name=@wishlistname AND W.serial_no=@serial) ))
BEGIN
SET @check='0'
END
ELSE
...
WHERE serial_no=@serial))
SET @check='1'
ELSE IF(EXISTS(SELECT * FROM Wishlist_Product W
WHERE W.customer_username=@customername AND W.wish_name=@wishlistname AND W.serial_no=@serial))
SET @check='2'
...
-------------------------------
GO
CREATE PROC checkRemovefromCart
 @customername VARCHAR(20), @serial int, @check BIT OUTPUT
AS
if (EXISTS (SELECT * FROM CustomerAddstoCartProduct
WHERE username=@customername AND serial_no=@serial))
SET @check='1'
ELSE
SET @check='0'
...
SET @check='0'
GO
CREATE PROC checkwishlistexist

@customername VARCHAR(20), @name VARCHAR(20),@check BIT OUTPUT
AS
 IF(EXISTS(SELECT * FROM Wishlist w WHERE @customername=w.username AND @name=w.name))
 SET @check='1'
 ELSE
 SET @check='0'
 ---------------------------